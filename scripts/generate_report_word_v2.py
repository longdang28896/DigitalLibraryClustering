# -*- coding: utf-8 -*-
"""
Script tạo file Báo Cáo Bài Tập Lớn (.docx) tinh gọn, chuẩn học thuật
...
Nhóm sinh viên:
- Đặng Hoàng Nhật Long - Nhóm trưởng
- Hoàng Văn Chính
- Trịnh Việt Cường
- Lê Văn Hiệp
Thời gian dự án: 05/09/2024 - 22/09/2024
"""
...
def format_student_table(table, col_widths, headers, data, align_cols=None):
...
def build_concise_report():
    ...
    def p(text="", bold_prefix="", indent=True, align=WD_ALIGN_PARAGRAPH.JUSTIFY, italic=False):
    ...
    # ==================== PHẦN TIÊU ĐỀ ĐẦU FILE (KHÔNG KHUNG) ====================
    ...
    r1 = top_p.add_run("BỘ GIÁO DỤC VÀ ĐÀO TẠO\nTRƯỜNG ĐẠI HỌC ...\nKHOA CÔNG NGHỆ THÔNG TIN\n")
    r1.bold = True
    r1.font.size = Pt(12)
    ...
    top_line.paragraph_format.space_after = Pt(18)
    r_line = top_line.add_run("--------------------***--------------------")
    ...
    r_main = title_p.add_run("BÁO CÁO BÀI TẬP LỚN\nHỌC PHẦN: KHAI PHÁ DỮ LIỆU VÀ MÁY HỌC (DMML)\n")
    r_main.bold = True
    r_main.font.size = Pt(14)
    
    sub_title = doc.add_paragraph()
    sub_title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    sub_title.paragraph_format.space_before = Pt(4)
    sub_title.paragraph_format.space_after = Pt(18)
    r_sub = sub_title.add_run("ĐỀ TÀI SỐ 2508: PHÂN CỤM TÀI LIỆU HỌC THUẬT CHO HỆ THỐNG THƯ VIỆN SỐ TRÊN NỀN TẢNG JAVA")
    r_sub.bold = True
    r_sub.font.size = Pt(15)

    # Danh sách thành viên thực hiện
    mem_p = doc.add_paragraph()
    mem_p.alignment = WD_ALIGN_PARAGRAPH.LEFT
    mem_p.paragraph_format.space_before = Pt(6)
    mem_p.paragraph_format.space_after = Pt(3)
    r_mem_t = mem_p.add_run("Nhóm sinh viên thực hiện:")
    r_mem_t.bold = True
    r_mem_t.font.size = Pt(13)
    ...
    time_p.paragraph_format.first_line_indent = Cm(0.8)
    time_p.paragraph_format.space_before = Pt(4)
    time_p.paragraph_format.space_after = Pt(18)
    r_time = time_p.add_run("Thời gian thực hiện dự án: Từ 05/09/2024 đến 22/09/2024")
    r_time.font.italic = True
    r_time.font.size = Pt(12)
    ...
    p("Dự án được thực hiện trong khoảng thời gian từ ngày 05/09/2024 đến ngày 22/09/2024 với sự phân công cụ thể giữa các thành viên như sau:")
    
    plan_headers = ["Thời gian", "Nhiệm vụ cụ thể", "Thành viên phụ trách", "Kết quả bàn giao"]
    plan_data = [
        ["05/09 - 08/09", "Khảo sát yêu cầu bài toán, thu thập tập dữ liệu 20 Newsgroups và 10 tài liệu tiếng Việt", "Đặng Hoàng Nhật Long\nHoàng Văn Chính", "Bộ dữ liệu JSON và tập tin mẫu .pdf, .docx, .txt"],
        ["09/09 - 12/09", "Module trích xuất PDF/DOCX (PDFBox, POI) và tiền xử lý NLP tiếng Việt (Unicode regex, stopwords)", "Hoàng Văn Chính\nTrịnh Việt Cường", "Lớp DocumentFileReader.java, TextPreprocessor.java"],
        ["13/09 - 16/09", "Không gian vector TF-IDF, cài đặt 3 thuật toán K-Means, HAC, DBSCAN và tính toán các chỉ số {Mi}", "Trịnh Việt Cường\nLê Văn Hiệp", "Mô hình K-Means Cosine, bộ tính Silhouette, DB, Purity"],
        ["17/09 - 20/09", "Thiết kế giao diện Java Swing FlatLaf, tích hợp mô hình vào 4 chức năng và tính năng sửa cụm", "Đặng Hoàng Nhật Long\nLê Văn Hiệp", "Giao diện 4 Tabs hoàn chỉnh, chức năng sửa cụm hoạt động"],
        ["21/09 - 22/09", "Kiểm thử toàn diện hệ thống (Unit tests 8/8 pass), đóng gói file JAR và hoàn thiện báo cáo BTL", "Cả nhóm", "File DigitalLibrary.jar, run.bat và Báo cáo BTL"]
    ]
    tbl_plan = doc.add_table(rows=1, cols=4)
    format_student_table(tbl_plan, [2.2, 5.5, 3.8, 4.5], plan_headers, plan_data, [WD_ALIGN_PARAGRAPH.CENTER, WD_ALIGN_PARAGRAPH.LEFT, WD_ALIGN_PARAGRAPH.LEFT, WD_ALIGN_PARAGRAPH.LEFT])
    ...
    p("Bảng số liệu đánh giá theo các chỉ số {Mi} với các thuật toán đã chọn trên tập dữ liệu 900 tài liệu:")
    ...
    format_student_table(tbl_eval, [4.2, 1.8, 2.5, 2.5, 2.5, 2.5], eval_headers, eval_data, 
    ...
    code_line("java -jar DigitalLibrary.jar")
    p("• Cách 3 (Biên dịch từ mã nguồn Maven):")
    code_line("mvn compile exec:java -Dexec.mainClass=\"com.dmml.library.DigitalLibraryApplication\"")

    h2("Phụ lục B: Cấu trúc thư mục dự án")
    code_line("DigitalLibraryClustering/")
    code_line("├── pom.xml                                   # Cấu hình thư viện Maven")
    code_line("├── run.bat                                   # File khởi chạy 1-Click chuẩn UTF-8")
    code_line("├── DigitalLibrary.jar                        # Bản phân phối thực thi JAR độc lập")
    code_line("├── data/")
    code_line("│   ├── 20newsgroups_sample.json              # 900 tài liệu học thuật tiếng Anh")
    code_line("│   ├── sample_vietnamese_docs.json           # 10 bài báo học thuật tiếng Việt")
    code_line("│   ├── stopwords_en.txt / stopwords_vi.txt   # Danh mục từ dừng en/vi")
    code_line("│   ├── experiment_results.json               # Số liệu thực nghiệm đối sánh mô hình")
    code_line("│   └── documents/                            # Kho tài liệu file vật lý thực tế (.pdf, .docx)")
    code_line("└── src/main/java/com/dmml/library/")
    code_line("    ├── DigitalLibraryApplication.java        # Điểm khởi chạy ứng dụng")
    code_line("    ├── util/DocumentFileReader.java          # Module trích xuất file PDFBox/POI")
    code_line("    ├── ml/nlp/TextPreprocessor.java          # Tiền xử lý NLP Unicode tiếng Việt")
    code_line("    ├── ml/feature/TfIdfVectorizer.java       # Không gian vector TF-IDF chuẩn hóa L2")
    code_line("    ├── ml/clustering/KMeansClusterer.java     # Thuật toán K-Means++ Cosine")
    code_line("    ├── ml/evaluation/                        # Silhouette, Davies-Bouldin, Purity")
    code_line("    ├── service/                              # DocumentRepository, ClusteringService")
    code_line("    └── ui/                                   # Giao diện Swing FlatLaf (4 Tabs & Dialogs)")
